...
rm testvec.dnak-htpg.mode40.dat
...
for m in `seq 1 50` ; do
	...
	awk -v num=$m 'BEGIN {
		...
		ifile2="ev-sorted-xyz_dnak-htpg-mode40.dat"
		rc=getline<ifile2
		k=1
		while (rc==1)
		{
		...
		paste testvec.add.$m.dat >> testvec.dnak-htpg.mode40.dat

done
for ((n=50; n>=1; n--)); do
	echo n=$n
	paste testvec.add.$n.dat >> testvec.dnak-htpg.mode40.dat
done
